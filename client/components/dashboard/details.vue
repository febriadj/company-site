<template>
  <div :class="[$style.details, details.isOpen && $style.active]">
    <div :class="[$style['details-wrap'], details.isOpen && $style.active]">
      ...
      <div :class="$style.attachment" v-if="news.attachment">
        <a :href="news.attachment" :download="news.attachment">
          <i class="bx bx-file" :class="$style.icon"></i>
          <p :class="$style.text">{{ news.attachment }}</p>
        </a>
      </div>
    </div>
  </div>
